<template>
  <jroll-infinite6 class="scroll-wrapper" id="wrapper6">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created(){
      ...
      this.$http.post('http://'+this.$store.state.urlIp+'api/index/borrow_comment/id/'+this.id,{label:4}).then(response=>{
    ...
    components:{
      'v-start' : stars,
      'jroll-infinite6': JRoll.VueInfinite({
        ...
        bottomed: function (complete) {
          var me = this
          var page =  _this6.t_page  //获取后台传来的数据页数

          if(typeof complete === 'function'){
            me.tip = '&nbsp;'
            page = 2
             _this6.ments = []
             _this6.nexturl = 'http://'+ _this6.$store.state.urlIp+'api/index/borrow_comment/id/'+ _this6.id+'/p/1'
          }
          if (me.page <  page - 1) {

             _this6.$http.post( _this6.nexturl,{label:3}).then(response => {
              let res = response.body
              let jons = res.info
               _this6.nexturl = res.nexturl
              if (typeof complete === 'function') {
                 _this6.ments = _this6.ments.concat(jons)
                me.page = 0
